...
def main():
    ...
    for username in args.usernames:
        try:
            logger.info(
                f"Collecting threads for @{username} (offline={settings['use_offline']})"
            )
            raw_items = scraper.fetch_user_threads(
                username=username, limit=settings["limit"]
            )
            parsed_items = [
    ...
    processed_path = exporter.to_csv(
        all_results, filename="clean_threads.csv", subdir="data/processed"
    )
